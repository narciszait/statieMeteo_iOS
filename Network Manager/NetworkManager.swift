...
class NetworkManager {
    func getNewestMeteos(completion: @escaping (Result<[Meteo], NetworkError>) -> Void) {
...
//        request.setValue("application/json", forHTTPHeaderField: "Accept")
        ...
        URLSession.shared.dataTask(with: request) {data, response, error in
            ...
            let meteos = try? JSONDecoder().decode([Meteo].self, from: data)
            meteos == nil ? completion(.failure(.decodingError)) : completion(.success(meteos!))
        }.resume()
    }
